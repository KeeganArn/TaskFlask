...
const CrmCompanies: React.FC = () => {
  ...
  const [form, setForm] = useState<{ name: string; domain?: string }>({ name: '' });

  const load = async () => {
    try {
      setLoading(true);
      setItems(await crmApi.listCompanies());
    } catch (e: any) {
  ...
  const create = async () => {
    if (!form.name) return;
    try {
      const created = await crmApi.createCompany(form);
    ...
    <div>
      ...
      <div className="mt-4 bg-white p-4 rounded shadow">
        <div className="grid grid-cols-2 gap-2">
          ...
          <input className="border rounded px-3 py-2" placeholder="Domain (optional)" value={form.domain || ''} onChange={e => setForm({ ...form, domain: e.target.value })} />
        </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {items.map((c) => (
                <tr key={c.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{c.domain || '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
